...
- **Redirect URLs:** Nhost dashboard must allow redirect URLs for OAuth, password reset, and email verification. For **Dropiti** specifically, add:
  - `https://yourdomain.com` and `http://localhost:3000` (app origin)
  - `https://yourdomain.com/auth/change-password` (password reset link target)
  - `https://yourdomain.com/auth/user-verification` (email verification link target; with `?verified=1` when used as redirectTo for resend)
  - `https://yourdomain.com/auth/signin` (OAuth return)
  Add the same paths with `http://localhost:3000` for local development.
